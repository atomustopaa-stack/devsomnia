// src/components/hero/HeroCompanyProfile.jsx
import { Link } from "react-router-dom";
import { ArrowRight, Sparkles, CheckCircle2, Building2 } from "lucide-react";

export default function HeroCompanyProfile({ bg = "#0b173d" }) {
  return (
      ...
      <div className="relative max-w-screen-xl mx-auto px-4 md:px-8 py-20 lg:py-24 grid lg:grid-cols-2 gap-12 items-center">
        ...
        <div className="[&>*]:relative">
          <span className="inline-flex items-center gap-2 text-sky-200/90 text-sm md:text-base">
            <Sparkles className="w-4 h-4" /> Jasa Website Company Profile
          </span>

          <h1 className="mt-3 text-3xl sm:text-4xl lg:text-5xl font-bold leading-tight">
            Bangun kredibilitas bisnis dengan
            <span className="block font-semibold text-sky-300">
              Company Profile profesional & meyakinkan
            </span>
          </h1>

          <p className="mt-5 max-w-2xl text-base sm:text-lg leading-relaxed text-sky-100/90">
            Tampilkan profil perusahaan, layanan, dan portfolio Anda dengan struktur rapi, performa cepat,
            serta siap tampil di Google. Desain konsisten dengan brand dan fokus pada hasil.
          </p>

          <ul className="mt-6 grid sm:grid-cols-2 gap-3 max-w-xl">
            {["Desain kredibel", "Struktur konten rapi", "SEO dasar siap", "Kecepatan optimal"].map((item) => (
        ...
        <div className="relative lg:pl-6">
          ...
          <figure className="relative aspect-[4/3] rounded-3xl overflow-hidden ring-1 ring-white/10 shadow-2xl">
            ...
            <img
              src="/image/hero/dymmy.1.jpg"
              alt="Ilustrasi website company profile profesional"
            ...
            <figcaption className="absolute bottom-3 right-4 text-xs text-white/70 backdrop-blur-sm bg-sky-900/30 rounded-full px-3 py-1">
              Preview tampilan company profile
            </figcaption>
          </figure>

          {/* Badge */}
          <div className="mt-4 flex items-center gap-2 text-sky-200/80">
            <Building2 className="w-4 h-4" />
            <span className="text-sm">Devsomnia — Website untuk bisnis yang serius tumbuh</span>
          </div>
        </div>
      </div>
    </section>
